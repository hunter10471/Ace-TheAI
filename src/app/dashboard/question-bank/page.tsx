...
export default function QuestionBankPage() {
  ...
  const [bookmarkedQuestions, setBookmarkedQuestions] = useState<number[]>([1]);
  ...
  const getDifficultyIcon = (difficulty: string) => {
    switch (difficulty) {
      case "Novice": return <LuLeaf className="w-4 h-4" />;
      case "Advanced": return <LuWrench className="w-4 h-4" />;
      case "Hard": return <LuTrophy className="w-4 h-4" />;
      default: return <LuLeaf className="w-4 h-4" />;
    }
  };

  const getCategoryIcon = (category: string) => {
    switch (category) {
      case "Technical": return <LuCog className="w-4 h-4" />;
      case "Behavioral": return <LuUsers className="w-4 h-4" />;
      case "Situational": return <TbUserExclamation className="w-4 h-4" />;
      default: return <LuCog className="w-4 h-4" />;
    }
  };
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="space-y-1">
                    {["Novice", "Advanced", "Hard"].map((difficulty) => (
                      <label key={difficulty} className="flex items-center cursor-pointer">
                        ...
                        <div className="flex items-center justify-between py-1 px-2 rounded w-full hover:bg-gray-50">
                          <div className="flex items-center">
                            ...
                            <span className={`text-xs ${selectedDifficulty === difficulty ? "font-semibold" : "font-normal text-gray-600"}`}>
                              {difficulty}
                            </span>
                          </div>
                          {selectedDifficulty === difficulty && <FaCheck className="w-3 h-3 text-primary" />}
                        </div>
                      </label>
                    ))}
                  </div>
                      ...
                      <label key={category} className="flex items-center cursor-pointer">
                        ...
                        <div className="flex items-center justify-between py-1 px-2 rounded w-full hover:bg-gray-50">
                          <div className="flex items-center">
                            ...
                            <span className={`text-xs ${selectedCategory === category ? "font-semibold" : "font-normal text-gray-600"}`}>
                              {category}
                            </span>
                          </div>
                          {selectedCategory === category && <FaCheck className="w-3 h-3 text-primary" />}
                        </div>
                      </label>
                    ))}
                  </div>
                ...
                <div className="pt-4 border-t border-gray-200">
                  <button className="flex items-center space-x-2 text-gray-700 hover:text-primary transition-colors">
                    ...
                    <span className="text-xs text-left">Saved Questions</span>
                  </button>
                </div>
              </div>
          </div>
              ...
              <div className="p-4 flex-1 overflow-y-auto">
                <div className="space-y-3">
                  {currentQuestions.map((question, index) => (
                    <div key={question.id} className={`pb-3 ${index !== currentQuestions.length - 1 ? "border-b border-gray-200" : ""}`}>
                      <div className="flex justify-between items-start">
                          ...
                          <h3 className=" text-gray-900 mb-1.5 text-sm leading-relaxed">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <button 
                                    ...
                                    >
                                    <span className="text-sm font-bold">View Explanation</span>
                        ...
                        <button
                          ...
                          className={`ml-3 ${bookmarkedQuestions.includes(question.id) ? "text-primary" : "text-gray-400"} hover:text-primary transition-colors`}
                        >
                          <FaBookmark className={`w-4 h-4 ${bookmarkedQuestions.includes(question.id) ? "fill-current" : ""}`} />
                        </button>
                      </div>
                    </div>
                  ))}
                ...
                <div className="flex justify-center items-center space-x-1">
                  ...
                  {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    ...
                    if (pageNum === 1 || pageNum === totalPages || (pageNum >= currentPage - 1 && pageNum <= currentPage + 1)) {
                      ...
                      return <span key={pageNum} className="px-1.5 text-gray-500 text-xs">...</span>;
                    }
                    return null;
                  })}
